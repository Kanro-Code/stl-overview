...
const prepOutput = () => {
	document
		...
		.addEventListener('click', (e) => {
			...
			if (output) {
				text.value = output
				document.querySelector('#outputAbsolute').checked = true
			}
		})
	...
	document
		...
		.addEventListener('input', (e) => {
			document.querySelector('#outputRelative').checked = true
		})
...
const pullSettings = () => {
	const conf = {
		...
		output: pullValueRadio('outputLocation')
	}
...
const loadPreviousSettings = () => {
	if (fs.existsSync(saveLocation)) {
		...
		if (settings.absolute) {
			document.querySelector('#outputLocationAbsolute').value = settings.absolute
		}
		if (settings.relative) {
			document.querySelector('#outputLocationRelative').value = settings.relative
		}
		if (settings.output === 'relative') {
			document.querySelector('#outputRelative').checked = true
		} else {
			document.querySelector('#outputAbsolute').checked = true
		}
